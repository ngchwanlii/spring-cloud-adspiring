...
import com.adspiring.ad.dto.TableTemplate;
import com.adspiring.ad.mysql.TemplateHolder;
...
public class AggregationListener implements BinaryLogClient.EventListener {
    ...
    public void onEvent(Event event) {
        ...
        // check if table & database name is already completed
        if (StringUtils.isEmpty(dbName) || StringUtils.isEmpty(tableName)) {
            log.error("no meta data");
            return;
        }

        // search table-related listener
    ...
    private BinlogRowData buildRowData(EventData eventData) {
        ...
        for (Serializable[] after : getAfterValues(eventData)) {
            ...
            for (int i = 0; i < colLen; ++i) {
                // retrieve current pos's column name
                String colName = table.getPosMap().get(i);

                // if doesn't exists mean this col is ignored
                if (colName == null) {
